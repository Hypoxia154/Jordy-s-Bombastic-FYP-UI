...
def list_users(current=Depends(get_current_user), _=Depends(check_permission)) -> list[UserPublic]:
    # require_role(current, allowed={"admin", "master"})
    return UsersRepository().list_users()

@router.post("", response_model=UserPublic)
def create_user(payload: UserCreateRequest, current=Depends(get_current_user), _=Depends(check_permission)) -> UserPublic:
    # require_role(current, allowed={"admin", "master"})
...
def update_role(username: str, payload: RoleUpdateRequest, current=Depends(get_current_user), _=Depends(check_permission)) -> UserPublic:
...
def delete_user(username: str, current=Depends(get_current_user), _=Depends(check_permission)) -> dict:
    # require_role(current, allowed={"master"})
