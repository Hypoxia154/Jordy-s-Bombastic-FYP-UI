...
<div style="font-size:0.95rem;font-weight:600;color:#f3f4f6;">{html.escape(user.get('name', 'User'))}</div>
...
def chat_bubble(role: str, content: str, sources=None, confidence=None, timestamp: str = "") -> None:
    ...
    if role == "user":
        ...
        ts_html = (
            f"<div style='font-size:0.7rem;color:rgba(255,255,255,0.5);text-align:right;margin-top:0.3rem;'>{safe_timestamp}</div>"
            if timestamp
            else ""
        )
...
<div style="background: linear-gradient(135deg, #2563EB 0%, #1D4ED8 100%); color: white; padding: 0.8rem 1.2rem; border-radius: 16px 16px 2px 16px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); font-size: 0.95rem; line-height: 1.5;">{safe_content}</div>
{ts_html}
</div>
</div>""",
            unsafe_allow_html=True,
  ...
  <div style="width:{pct}%;height:4px;background:{color};border-radius:2px;"></div>
            ...
            if sources:
                chips = "".join(
                    ...
                    f'border-radius:999px;font-size:0.75rem;color:#93c5fd;">{html.escape(str(s))}</span>'
                    for s in sources
                )
                st.markdown(
            ...
            if timestamp:
                st.markdown(
                    f"<div style='font-size:0.7rem;color:#4b5563;margin-top:0.3rem;'>{html.escape(timestamp)}</div>",
                    unsafe_allow_html=True,
                )
