...
class TestFollowupResolution:
    def test_followup_uses_last_resolved_query_from_session_state(self):
        service, mock_llm, _ = build_service()
        result = service._rewrite_dependent_query(
            "elaborate more",
            history=["User: What is the booking fee?", "Assistant: The booking fee is refundable in some cases."],
            session_state={"last_resolved_query": "what is the booking fee in the tenancy agreement"},
            file_filter=None,
        )
        assert result == "what is the booking fee in the tenancy agreement"

    def test_build_rag_plan_assesses_with_resolved_query(self):
        service, mock_llm, _ = build_service()
        mock_llm.complete.side_effect = [
            MagicMock(text="DEPENDENT"),
            MagicMock(text="What is the booking fee in the tenancy agreement?"),
        ]

        fake_nodes = [make_fake_node("The booking fee is part of the upfront payment terms.", score=5.0)]
        mock_retriever = MagicMock()
        mock_retriever.retrieve.return_value = fake_nodes

        with patch("app.services.crag_service.VectorIndexRetriever", return_value=mock_retriever):
            with patch.object(service, "_assess_answerability", wraps=service._assess_answerability) as wrapped_assess:
                plan = service.build_rag_plan(
                    "elaborate more",
                    history=[
                        "User: What is the booking fee?",
                        "Assistant: The booking fee is part of the upfront payment terms.",
                    ],
                    session_state={"last_resolved_query": "what is the booking fee"},
                )

        assert plan["intent"] == "DOMAIN"
        assert wrapped_assess.call_args.kwargs["query"] == "what is the booking fee"
        assert plan.get("resolved_query") == "what is the booking fee"
