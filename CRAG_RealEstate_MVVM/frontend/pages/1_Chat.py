...
with st.sidebar:
    ...
    if not sessions:
        ...
        for s in sessions:
            label = f"{s.get('title','(untitled)')} • {s.get('created_at','')}"
    ...
    if uploaded is not None:
        if st.button("Ingest", use_container_width=True):
            st.info("Upload UI is ready — next step is wiring to FastAPI.")
...
if question:
    with st.spinner("CRAG is retrieving relevant knowledge..."):
        try:
            ...
            st.session_state["active_session_id"] = resp.get("session_id") or active_session_id
        except Exception as e:
