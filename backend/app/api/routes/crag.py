...
def query(
    ...
    session_updates = dict(result_dict.get("session_updates") or {})
    ...
    if session_updates and (answer_text not in {service.ADMIN_UPDATE_MESSAGE, service.KL_SCOPE_MESSAGE} or session_updates.get("user_name") or session_updates.get("last_user_query")):
        repo.patch_session_state(session_id, session_updates)
...
def query_stream(
    ...
    def event_gen():
        ...
        if direct_answer is not None:
            running = service._enforce_safe_output(str(direct_answer))
        ...
        if evidence:
            # pass the full evidence array and the original question to the new grounding method
            bleu_score = service._compute_grounding_score(running, evidence, payload.question)
        else:
            bleu_score = 0.0
        ...
        if running not in {service.ADMIN_UPDATE_MESSAGE, service.KL_SCOPE_MESSAGE}:
            session_updates["last_intent"] = intent
        if direct_answer is None and running and running not in {service.ADMIN_UPDATE_MESSAGE, service.KL_SCOPE_MESSAGE}:
            resolved_query = plan.get("resolved_query") or payload.question
            session_updates.update(
                service._build_session_state_updates(
                    raw_query=payload.question,
                    resolved_query=resolved_query,
                    answer_text=running,
                    task_type=service._detect_task_type(resolved_query, file_filter=payload.file_filter if hasattr(payload, "file_filter") else None),
                    file_filter=payload.file_filter if hasattr(payload, "file_filter") else None,
                    evidence=evidence,
                )
            )
        elif direct_answer is not None and running and running not in {service.ADMIN_UPDATE_MESSAGE, service.KL_SCOPE_MESSAGE}:
            session_updates["last_answer_summary"] = service._sanitize_answer_text(running)[:500]
        if session_updates and (running not in {service.ADMIN_UPDATE_MESSAGE, service.KL_SCOPE_MESSAGE} or session_updates.get("user_name") or session_updates.get("last_user_query")):
            repo.patch_session_state(session_id, session_updates)
